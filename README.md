...
When `-enable-migration` is combined with `-proxy-jump`, the migration coordinator runs on the client↔proxy leg only.  The target leg's transport always sends to the loopback end of the proxy's UDP-forward, a "path" that never goes down, so a second coordinator on the target leg would have nothing useful to migrate to — at best it would open a fresh kernel UDP socket and tunnel a duplicate PATH_CHALLENGE through the (already-migrating) proxy, doubling the UDP-forward flow on the proxy for no benefit.  When the proxy leg switches paths the target leg's loopback traffic continues to ride that switched path transparently, so one coordinator is enough to recover both legs from a network change.

**Caveat:** proxy-jump migration is currently best-effort and not yet covered by an automated end-to-end test (the direct client↔server migration spec passes, but the proxy-jump variant is `PIt`-pending while the test harness is reworked into a three-namespace topology).  Treat it as a feature to validate against your own setup before relying on it in production.
